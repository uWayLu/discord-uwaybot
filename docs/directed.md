...
### 接龍／捏他偵測（`maybeReference`）
- 過 `directed-gate` 後，`@bot` 若收到**短句（2~80 字、無網址/指令）**或**含明確搜尋要求**（`你找一下…` 等），會先 `searchWithContent` 抓搜尋結果＋第一筆網頁內文，再餵 `referenceReply`：
  - 是台詞/咒文/歌詞且有下一句 → 接龍。
  - 是迷因/名句（如 `會贏喔`→五條悟）→ 用角色口吻或 meme 句型回（如「你五條{author}?」）。
  - 是明確搜尋要求 → 依內文給簡短有根據的答案。
